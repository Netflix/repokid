...
class ExclusiveFilter(Filter):
    def __init__(self, config=None):
        ...
        exclusive_role_globs.update([role_glob.lower() for role_glob in config.get(current_account, [])])
        exclusive_role_globs.update([role_glob.lower() for role_glob in config.get('all', [])])
    ...
    def apply(self, input_list):
        ...
        for role_glob in self.exclusive_role_globs:
            exclusive_roles += [role for role in input_list if fnmatch.fnmatch(role.role_name.lower(), role_glob)]
