...
def list_repoable_services(dynamo_table, message):
    ...
    if not role_id:
        return ResponderReturn(successful=False,
                               return_message='Unable to find role {} in account {}'.format(message.role_name,
                                                                                            message.account))
    else:
...
def opt_out(dynamo_table, message):
    ...
    if not message.reason or not message.requestor:
        return ResponderReturn(successful=False, return_message='Reason and requestor must be specified')
    ...
    if 'OptOut' in role_data and role_data['OptOut']:
        ...
        timestr = time.strftime('%m/%d/%y', time.localtime(role_data['OptOut']['expire']))
        return ResponderReturn(successful=False,
                               return_message=('Role {} in account {} is already opted out by {} for reason {} '
...
def remove_opt_out(dynamo_table, message):
    ...
    if 'OptOut' not in role_data or not role_data['OptOut']:
        return ResponderReturn(successful=False,
                               return_message='Role {} in account {} wasn\'t opted out'.format(message.role_name,
                                                                                               message.account))
    else:
        dynamo.set_role_data(dynamo_table, role_id, {'OptOut': {}})
        return ResponderReturn(successful=True,
                               return_message='Cancelled opt-out for role {} in account {}'.format(message.role_name,
                                                                                                   message.account))
